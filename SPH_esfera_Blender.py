...
def atualizar_posicao(esferas, x, y, z):
    for i in range(len(esferas)):
        esferas[i].location = (x[i], y[i], z[i])

bpy.context.scene.frame_start = 0
bpy.context.scene.frame_end = num_frames

esferas = criar_esferas(n)

for frame in range(num_frames):
    bpy.context.scene.frame_set(frame)
    x, y, z, vx, vy, vz = atualizar_rk4(x, y, z, vx, vy, vz, M, h, dt)
    atualizar_posicao(esferas, x, y, z)
    for esfera in esferas:
        esfera.keyframe_insert(data_path="location", frame=frame)
